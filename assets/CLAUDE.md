...
- **JustLoop** — **Retired on 2026-09-24**, with no successor. Dropped an audio file and looped it, from the menu bar. The GitHub repo is archived and its local clone deleted, so the audits no longer list it; no features, no releases.
- **JustIn** — **Retired on 2026-09-24**; WaxOn/WaxOff covers it. A media file analyzer. The GitHub repo is archived and its local clone deleted, so the audits no longer list it, and installed copies keep reporting v1.0.4 as current, since its update check reads the archived releases.
...
- **Cypher** — **Retired on 2026-09-23**, with its Windows port. Passphrase-based text encryption; the repos are `sevmorris/FL2601` and `sevmorris/FL2601-Windows`, both archived. Nothing already encrypted with it may become unreadable, so what decrypts stays up: the web version (`sevmorris.github.io/FL2601/web/`, served from the archived repo's Pages), the Theory of Operation that documents the format, and the last release of each app. The `fl2601` cask is deprecated, and Homebrew disables it on 2027-09-23. Its local clone is deleted, so the audits no longer list it; no features, no releases.
...
- **Homebrew is no longer a distribution channel for these apps.** The `doublender` and `waxonwaxoff` casks were deprecated on 2026-09-17: both apps check GitHub for their own updates, so the cask was a second update path whose records went stale the moment an in-app update was installed — Homebrew had 2.12.0 and 2.5.0lr recorded against 2.12.4 and 2.5.4lr on disk, and nothing noticed. They are deprecated rather than deleted so an existing `brew upgrade` says where the app went. `fl2601` was the one exception, because Cypher is sandboxed with no network entitlement and cannot check for its own updates, until Cypher was retired on 2026-09-23 and its cask deprecated too. Homebrew disables a deprecated cask a year after its date. Do not add a cask-bump step back to any release script.
...
- `./build.sh` + `./distribute.sh` — PasswordGen.
...
- Deployment targets: 14.0 for WaxOnWaxOff, ClipHack, FilmStrip and Barkeep; 13.0 for DoublEnder; 15.0 for KeyVault.
...
- `PBXFileSystemSynchronizedRootGroup` in WaxOnWaxOff, ClipHack, FilmStrip and KeyVault — new files in a synced folder are picked up with no project edit. Barkeep and DoublEnder do **not** use it, so adding or removing a file there means editing `project.pbxproj` (or `project.yml`, for DoublEnder).
